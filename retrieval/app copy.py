...
CONFIG = {
    ...
    "ground_truth_path": "/Users/muhammadzuamaalamin/Documents/RisetTextMining/retrieval/eval_ground_truth2.json",
...
def rewrite_query(query: str, llm=None) -> str:
    """
    Rewrite query ke bahasa hukum formal menggunakan LLM.
    Jika llm=None, kembalikan query asli (fallback aman).
    """
    if llm is None:
        return query

    prompt = f"""
Kamu adalah asisten sistem pencarian dokumen hukum.

Tugasmu adalah memperbaiki query pencarian agar lebih mudah
menemukan pasal dalam UUD 1945.

Aturan:
- Jangan mengubah makna pertanyaan
- Jangan menambahkan nomor pasal jika tidak disebutkan
- Pertahankan kata kunci utama dari pertanyaan
- Jika perlu, tambahkan sinonim hukum yang relevan
- Maksimal 1 kalimat

Pertanyaan:
{query}

Query pencarian yang diperbaiki:
"""

    response = llm.invoke(prompt)
    rewritten = response.content.strip()

    logger.info("Query asli   : %s", query)
    logger.info("Query rewrite: %s", rewritten)
...
def run_evaluation(cfg: dict) -> dict:
    ...
    # Inisialisasi LLM untuk query rewriting (set None untuk skip)
    rewrite_llm = ChatOllama(model="gemma3:4b", temperature=0)
    # ── Evaluasi per query ────────────────────────────────────────────────
    for item in ground_truth:
        qid     = item["query_id"]
        query   = item["query"]
        rel_kws = item["relevant_chunks_keywords"]
        # Rewrite query ke bahasa hukum formal
        query_rewritten = rewrite_query(query, llm=rewrite_llm)

        logger.info("[%s] %s", qid, query)

        for method_name, retriever in retrievers.items():
            retrieved_texts = retriever.retrieve_texts(query_rewritten)
